...
import 'C:/Users/USER/Desktop/Zafran/meal-service/dashboard/src/styles/Payment.css';
...
const PaymentPage = () => {
  ...
  const availableOptions = ['PayPal', 'MasterCard', 'Apple Pay','VISA','MADA','American Express','Al Rajhi','STC Pay'];
  ...
  const handleAddOption = () => {
    if (newOption) {
      ...
      if (optionExists) {
        ...
        setPaymentOptions([...paymentOptions, { id, name: newOption, enabled: true }]);
        setNewOption(''); 
        setErrorMessage(''); 
      }
    }
        ...
        {paymentOptions.map((option) => (
          <div key={option.id} className="payment-item">
            ...
            <div className="controls">
              ...
              <button className="remove-btn" onClick={() => handleRemoveOption(option.id)}>
                Remove
              </button>
            </div>
          </div>
